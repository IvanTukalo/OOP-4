...
class Program
{
    ...
    static void Block1_Timer()
    {
        ...
        Timer timer1 = new Timer(() => Console.WriteLine($"[{DateTime.Now:T}] Абрикос 1"), 1);
        Timer timer2 = new Timer(() => Console.WriteLine($"[{DateTime.Now:T}] Абрикос 2"), 4);
    ...
    static void Block2_Array()
    {
        ...
        Func<int, bool> IsDiv = num => num % k == 0;

        // Варіант з LINQ
        int[] FiltredByLinq = arr.Where(IsDiv).ToArray();
